...
class DataAccess extends CI_Model {
	...
	public function getFiches ($idVisiteur) {
		$req = "select idVisiteur, mois, montantValide, dateModif, idEtat, libelle
		from  fichefrais inner join Etat on ficheFrais.idEtat = Etat.id
		where fichefrais.idvisiteur = '$idVisiteur'
		order by mois desc";
		$rs = $this->db->query($req);
	...
	public function getFichesV()
	{
		$req = "select idVisiteur, mois, montantValide, dateModif, idEtat, libelle from fichefrais inner join Etat E on idEtat = E.id where idEtat = 'CL'";
		$rs = $this->db->query($req);
		$lesFiches = $rs->result_array();
		return $lesFiches;
	}
